...
def create_folder_a(): #{
	win2_a = Toplevel(bg = "white")
	Label(win2_a, text="Please input the following data:", font=(font_type, size_variable, "bold"), bg="white").grid(row=0, column=0, columnspan=2, pady=(5,5))
	
	# Directory location
	Tkinter.Button(win2_a, text="Select Parent Diretory", font=(font_type, size_variable), bg="white", command=load_directory).grid(row=1, columnspan=2)
	
	# Date? - will be retrieved from desktop
	today = str(datetime.date.today()) # will return YEAR-MO-DA as a string
	
	Tkinter.Label(win2_a, text="Date:", font=(font_type, size_variable), fg = fg_color, bg="white", pady=2).grid(row=2, column=0, sticky='E')
	DateTxt2_a = Tkinter.Entry(win2_a, font=(font_type, size_variable), bg="white").grid(row=2, column=1)
	
	DateTxt2_a.delete(0, END)
	DateTxt2_a.insert(0, today)
	
	# Place?
	Tkinter.Label(win2_a, text="Location:", fg = fg_color, font=(font_type, size_variable), bg="white").grid(row=3, sticky='E')

	nameTxt2 = Tkinter.Entry(win2_a, font=(font_type, size_variable), bg="white").grid(row=3, column=1)
	
	Tkinter.Button(win2_a, text="Press to Generate Folder", font=(font_type, size_variable), bg="white", command=create_directory).grid(row=4, columnspan=2, sticky='WE', padx=5, pady=5)
#}

# ---------------------------------------------------------------------------------------------------------------------
def load_directory_a(): #{
	dirname = tkFileDialog.askdirectory(parent=root,initialdir="/",title='Please select a directory')
	if len(dirname ) > 0:
		inFileTxt_a.delete(0, END)
		inFileTxt_a.insert(0, dirname)
...
def create_directory(): #{
	...
	date_pathname_a = ''
	name_pathname_a = ''
	if len(inFileTxt_a.get()) == 0:
		inFile_pathname_a = DEFAULT_FILE_LOCATION
	else:
		inFile_pathname_a = inFileTxt_a.get()
	date_pathname_a = DateTxt2_a.get()
	if len(nameTxt2_a.get()) == 0:
		name_pathname_a = DEFAULT_NAME
	else:
		name_pathname_a = nameTxt2_a.get()
	path_a = inFile_pathname_a + date_pathname_a + '_' + name_pathname_a
	os.makedirs(path_a, 0755)
#}
...
def Zurich_asynch_SINGLE(): #{
	...
	win_a = Toplevel(bg = "white")
	Label(win, text="Taking data, please, wait 10 seconds", font=(font_type, size_variable), bg="white").grid(row=0, column=0, pady=(5,5))
